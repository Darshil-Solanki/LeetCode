class SegmentTree:
    ...
    def makeLeaf(self, o: int, value: int):
        info = [0] * (self.k + 1)
        r = value % self.k
        info[r] = 1
        info[self.k] = r
    ...
    def mergePre(self, left: List[int], right: List[int]):
        pre = [0] * (self.k + 1)
        mul_L = left[self.k]
        mul_R = right[self.k]
    ...
    def update(self, o: int, l: int, r: int, index: int, value: int):
        if l == r:
...
class Solution:
    def resultArray(self, nums: List[int], k: int, queries: List[List[int]]) -> List[int]:
        ...
        ans = []
        for i, value, start, x in queries:
            seg.update(1, 0, n-1, i, value)
